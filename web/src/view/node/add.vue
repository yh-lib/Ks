...
import { addnodeHandler as addHandler,updatenodeHandler as updateHandler } from '../../api/node.js'
...
const rules = reactive({
    nodeId: [{ required: true, message: '请输入集群ID', trigger: 'blur' },],
    nodeAlias: [{ required: true, message: '请输入集群名称', trigger: 'blur' }],
    nodeLocation: [{ required: true, message: '请输入集群地址', trigger: 'blur' }],
    nodeKubeconfig: [{ required: true, message: '请输入集群kubeConfig', trigger: 'blur' }],
})
// add_6 重置表单
const resetForm = () => {
...
<template>
    ...
    >
        ...
        <!-- 左半部分 -->
        <div>
            <el-form-item label="集群ID"  prop="nodeId">
                <el-input v-model="data.itemForm.nodeId" autocomplete="off" :readonly="props.subMethod == 'update'"/>
            </el-form-item>
            <el-form-item label="集群名称"  prop="nodeAlias">
                <el-input v-model="data.itemForm.nodeAlias" autocomplete="off" />
            </el-form-item> 
            <el-form-item label="集群位置"  prop="nodeLocation">
                <el-input v-model="data.itemForm.nodeLocation" autocomplete="off" />
            </el-form-item>                              
        </div>
        <!-- 右半部分 -->
        <div>
