<script lang="ts" setup>
  ...
  const deleteItem = (name) => {
    console.log('删除函数')
    ElMessageBox.confirm('确认删除命名空间：' + name, {
  ...
  const filterTableData = computed(() =>
    data.items.filter(
      (item) => !data.search || item.metadata.name.toLowerCase().includes(data.search.toLowerCase())
    )
  )
      ...
      <div class="card-header">
        ...
        <div>
          <el-select
            ...
            @change="getList(data.curClusterId)"
          >
            <el-option
          ...
          <el-button type="primary" @click="createItem()" style="width: 105px; margin-left: 10px"
            >创建</el-button
          >
        </div>
      </div>
    ...
    <el-table :data="filterTableData" style="width: 100%" height="70vh">
      ...
      <el-table-column label="操作" prop="operation">
        <template #default="scope">
          <el-button link type="danger" @click="deleteItem(scope.row.metadata.name)"
            >删除</el-button
          >
        </template>
      </el-table-column>
