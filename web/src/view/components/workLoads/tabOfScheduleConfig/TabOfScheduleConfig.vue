<script setup>
  ...
  // 节点标签
  const addNodeLabelItem = () => {
    data.nodeLabelsList.unshift({ key: '', value: '' })
  }
  const deleteNodeLabelItem = (index) => {
    data.nodeLabelsList.splice(index, 1)
  }
  // 容忍标签
  const addTolerationItem = () => {
    workLoadItem.value.item.spec.template.spec.tolerations.unshift({ key: '', value: '' })
  }
  const deleteTolerationItem = (index) => {
    workLoadItem.value.item.spec.template.spec.tolerations.splice(index, 1)
  }
</script>
