<script setup>
  ...
  const rules = reactive({
    ...
    items: {
      key: [{ required: true, message: '请输入secret Key', trigger: 'blur' }],
      path: [
    ...
    <el-tabs>
      <el-tab-pane label="基础配置">
        ...
        <el-form-item label="secret" prop="secret.secretName">
          <el-select
            v-model="data.volumeItem.secret.secretName"
            placeholder="请选择要挂载的secret"
        ...
        <el-form-item label="默认挂载权限" prop="secret.defaultMode">
          <el-input
            v-model.number="data.volumeItem.secret.defaultMode"
        ...
        <el-form-item label="可选" prop="secret.optional">
          <el-select
            v-model="data.volumeItem.secret.optional"
            placeholder="secret 缺失时是否允许 Pod 启动"
          >
            <el-option
          ...
          <el-table-column prop="mode" label="挂载权限" width="200px">
            <template #default="scope">
              <el-form-item
                label=""
                label-width="0px"
                :prop="'secret.items.' + scope.$index + '.mode'"
