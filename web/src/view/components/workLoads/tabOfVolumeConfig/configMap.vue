<script setup>
  ...
  import { getConfigMapListHandler } from '../../../../api/configMap.js'
  ...
  const rules = reactive({
    name: [{ validator: validateVolumeName, trigger: 'blur' }],
    items: {
      key: [{ required: true, message: '请输入configMap Key', trigger: 'blur' }],
      path: [
  ...
  // 添加自定义挂载路径
  const addMountPath = () => {
    data.volumeItem.configMap.items.push({
      key: '',
      path: '',
      mode: '',
    })
  ...
  const deleteMountPath = (index) => {
    data.volumeItem.configMap.items.splice(index, 1)
  }
    ...
    <el-tabs>
      <el-tab-pane label="基础配置">
        ...
        <el-form-item label="configMap" prop="configMap.name">
          <el-select
            v-model="data.volumeItem.configMap.name"
            placeholder="请选择要挂载的configMap"
        ...
        <el-form-item label="默认挂载权限" prop="configMap.defaultMode">
          <el-input
            v-model.number="data.volumeItem.configMap.defaultMode"
            placeholder="请输入默认挂载权限"
            @change="oct2Dec"
          />
        </el-form-item>
        <el-form-item label="可选" prop="configMap.optional">
          <el-select
            v-model="data.volumeItem.configMap.optional"
            placeholder="ConfigMap 缺失时是否允许 Pod 启动"
          >
            <el-option
      ...
      <el-tab-pane label="自定义挂载路径" class="no-border-input">
        <el-table :data="data.volumeItem.configMap.items">
          ...
          <el-table-column prop="mode" label="挂载权限" width="200px">
            <template #default="scope">
              <el-form-item
                label=""
                label-width="0px"
                :prop="'configMap.items.' + scope.$index + '.mode'"
                :rules="rules.items.mode"
              >
                <el-input
                  v-model.number="scope.row.mode"
                  placeholder="请输入挂载权限"
                  @change="oct2Dec(scope.row)"
                />
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column prop="operation" align="center" width="100px">
            ...
            <template #default="scope">
              <el-button type="danger" link size="large" @click="deleteMountPath(scope.$index)"
                >删除</el-button
              >
            </template>
          </el-table-column>
