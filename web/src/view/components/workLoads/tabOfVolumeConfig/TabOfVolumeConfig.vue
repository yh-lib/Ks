<script setup>
  ...
  // 选项列表
  const volumeTypeOptions = ['configMap', 'secret', 'emptyDir', 'hostPath', 'pvc', 'nfs']
  // 组件列表
...
<template>
  ...
  >
    <div style="display: flex; justify-content: left; padding: 0 70px; margin-top: 10px">
      ...
      <el-select
        v-model="data.volumeType"
        placeholder="请选择存储卷类型"
        size="large"
        style="width: 1070px; margin-left: 20px"
      >
        <el-option v-for="item in volumeTypeOptions" :key="item" :label="item" :value="item" />
      </el-select>
    </div>
    <!-- 动态选择组件 -->
    <component
    ...
    <template #footer>
      <div style="display: flex; justify-content: center">
        <el-button
          type="primary"
          style="width: 100px; height: 40px; margin-bottom: 10px; font-size: 14px"
          @click="addVolume"
        >
          添加
        </el-button>
